      ...
      >
        <span
          className={`text-lg font-semibold ${
            isOpen ? "text-primary-text" : "text-2ndcolor-text"
          }`}
        >
          {question}
        </span>
        {isOpen ? (
          <Minus size={24} className="text-primary-text" />
        ) : (
          <Plus size={24} className="text-2ndcolor-text" />
        )}
      </button>
      {isOpen && (
        <div
          id={`faq-answer-${question.replace(/\s/g, "-").toLowerCase()}`}
          className="mt-4 text-gray-700 pr-8"
        >
